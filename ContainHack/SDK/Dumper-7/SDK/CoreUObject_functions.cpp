...
namespace SDK
{
	...
	FVector& operator*=(const decltype(X) Scalar) const
	{
		X *= Scalar;
		Y *= Scalar;
		Z *= Scalar;
	...
	float Dot(const FVector& Other)
	{
		return (X * Other.X) + (Y * Other.Y) + (Z * Other.Z);
	}

	float FVector::MagnitudeSqr()
	{
		return Dot(*this);
	}
	...
	float FVector::Distance(const FVector& v)
	{
		return float(std::sqrtf(powf(v.X - X, 2.0) + std::powf(v.Y - Y, 2.0) + std::powf(v.Z - Z, 2.0)));
	}

	float FVector::DistanceMeter(FVector& v)
	{
